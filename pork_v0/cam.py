...
def convert_img2train(color_image,colorize_depth_image):
    color_image=color_image[:,:,:3]
    gray2=cv2.cvtColor(colorize_depth_image, cv2.COLOR_BGR2GRAY)
    canny1=cv2.Canny(color_image,50,150)
    canny1=cv2.cvtColor(canny1,cv2.COLOR_GRAY2BGR)
    canny2=cv2.Canny(gray2, 2,5)
    canny2=cv2.cvtColor(canny2,cv2.COLOR_GRAY2BGR)
    canny=cv2.addWeighted(canny2, 1, canny1, 1,0)
    new_img=cv2.addWeighted(canny, 1, color_image, 1,0)    
    return new_img
...
#     # load detecting model
#     VSBIG = detect.Visualization(m_path,0,["square","circle"])
#     # VSSMALL = detect.Visualization(m_path,1,[])
#     cv2.namedWindow('Azure Kinect DK Image', cv2.WINDOW_NORMAL)
...
#         # Get the colored depth
#         ret, depth_img = capture.get_transformed_depth_image()
#         if not ret:
#             continue
#         color_depth = capture.color_depth_image(depth_img)
...
#                 if conn:
#                     if pork_type==0:
#                         df_ins = VSBIG.run_on_image(new_img, idx)
#                     # elif pork_type==1:
...
#                         for j in range(10):
#                             capture = device.update()
#                             ret, xyz_array = capture.get_transformed_pointcloud()
#                             width=capture.camera_transform.color_resolution.width
#                             height=capture.camera_transform.color_resolution.height
#                             xyz_array=xyz_array.reshape(height,width,3)
